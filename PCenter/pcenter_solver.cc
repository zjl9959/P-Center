...
void PCenterSolver::LoadGraph(const string path) {
    if (path.rfind(".txt") != string::npos) {
    ...
    } else if (path.rfind(".tsp") != string::npos) {
...
void PCenterSolver::Solve() {
    cout << "---------------START SOLVER---------------" << endl;
    LoadGraph(string("instance/") + kInstanceName);
    GenInitSolution();
    TabuSearch();
    vector<int> result;
    result = GetResult();
    if (Check()) {
        ...
        cout << "Objective: " << best_objval_ << endl;
        cout << "Facilitys: ";
        for (int i = 0; i < result.size() - 1; ++i)
            cout << result[i] << ",";
        cout << result.back() << "\b\a" << endl;
    }
}

void PCenterSolver::Record(const char * log_path) {
    ofstream ofs(log_path,ios::app);
    if (ofs.is_open()) {
        ofs.seekp(0, ios::end);
        if (ofs.tellp() == 0)
            ofs << "StartTime,Algorithm,Instance,FacilityNum,TimeStamp,IterationStamp,GenerationStamp,Valid,Object,Configure(k/timeout/maxsteps/seed),Solution" << endl;
        ofs << FormatTime(kStartTime) << ","
            << kAlgorithm << ","
            << kInstanceName << ","
            << facility_num_ << ","
            << GetTimeStamp() << ","
            << info.iteration_stamp << ","
            << info.generation_stamp << ","
            << Check() << ","
            << best_objval_ << ","
            << cfg_.ToString() << ",";
        for (int i = 0; i < best_facility_nodes_.size() - 1; ++i)
            ofs << best_facility_nodes_[i] << " ";
        ofs << best_facility_nodes_.back() << endl;
        ofs.close();
    }
}

std::vector<int> PCenterSolver::GetResult() {
...
void PCenterSolver::GenInitSolution() {
    ...
    while (choosed_facility_num < facility_num_) {
        ...
        GetKthNeighbors(longest_service_node, cfg_.consider_range, neighborks);
...
void PCenterSolver::GetKthNeighbors(int node, int k, vector<int>& res) {
    int index = 0;
    while (k--) {
...
void PCenterSolver::FindMove(int k, int & choosed_user, int & choosed_facility) {
...
void PCenterSolver::MakeMove(int choosed_user, int choosed_facility) {
    ...
    if (current_objval_ < best_objval_) {
        best_objval_ = current_objval_;
        best_facility_nodes_ = facility_nodes_;
        info.iteration_stamp = iterator_num_;
        info.time_stamp = clock();
        cout << "A better obj: " << best_objval_ << " on iteration " << iterator_num_ << endl;
    }
}

void PCenterSolver::TabuSearch() {
    int choose_user, choose_facility;
    iterator_num_ = 1;
    while (!IsTimeOut() && iterator_num_ < cfg_.max_tabu_steps) {
        FindMove(cfg_.consider_range, choose_user, choose_facility);
        MakeMove(choose_user, choose_facility);
        iterator_num_++;
    }
}

const bool PCenterSolver::Check() const {
